...
def test_admin_achievement_rule_options(tmp_path: Path):
    ...
    assert "MINIGAME_PLAY" in payload["event_types"]
    assert "RECORD_ATTACHMENT" in payload["event_types"]
    assert "박자" in payload["tags"]
    assert "CORE" not in payload["tags"]
    assert "song_library_id" in payload["fields"]
    assert "record.post_id" in payload["fields"]
    ...
    assert "minigame.game" in payload["condition_fields"]
    assert "minigame.accuracy" in payload["condition_fields"]
    assert "record.media_type" in payload["condition_fields"]
    assert "record.video_upload_count" in payload["condition_fields"]
    ...
    assert "minigame.game" in payload["field_meta"]
    assert "minigame.score" in payload["field_meta"]
    assert "record.media_type" in payload["field_meta"]
